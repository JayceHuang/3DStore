...
)
{
	...
	while (dwLower <= dwUpper)
	{
		...
		compareResult = compare(key, pMiddleArrayElement, width);
		if (compareResult == 0)
		{
...
//
{
	...
	else
	{
		...
		// 
		wsprintf(wszMapFileName, L"\\%s", wszModuleName);
...
//
{
	...
	PMAPSYMBOL pNextSymbol = (PMAPSYMBOL)((char*)pSymbol + width);
	if (dwAddress < pSymbol->address)
		return -1;
	else if (dwAddress < pNextSymbol->address)
		return 0;
	else if ((pNextSymbol->address == NULL) && (dwAddress <= pSymbol->address + pSymbol->length - 1))
		return 0;
	else
		return 1;
}
