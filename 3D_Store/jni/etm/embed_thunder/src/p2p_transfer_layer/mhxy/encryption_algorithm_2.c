...
BOOL encryption_algorithm_create_key_2(ENCRYPTION_ALGORITHM *p_algorithm, const _u8 *data, _u32 len, void *key, _u32 *p_key_len )
{
    // 创建首部
    if (len == 0)
    {
        ...
        // 加密方法
        _u32 head = ((8192*2 + rand() % 8192) << 16) + rand() % 65536; // [0x40000000, 0x5FFFFFFF]
...
BOOL encryption_algorithm_encrypt_2(ENCRYPTION_ALGORITHM *p_algorithm, _u8 *in_buffer, _u32 len )
{
    _u32 next;
    _u32 i=0;
    for (i=0; i<len; i++)
...
BOOL encryption_algorithm_decrypt_2(ENCRYPTION_ALGORITHM *p_algorithm, _u8 *in_buffer, _u32 len )
{
    _u32 next;
    _u32 i=0;
    for (i=0; i<len; i++)
...
BOOL encryption_algorithm_2_new(struct tagENCRYPTION_ALGORITHM ** pp_algorithm)
{
    *pp_algorithm = (ENCRYPTION_ALGORITHM *)malloc(sizeof( ENCRYPTION_ALGORITHM ));
    memset(*pp_algorithm, 0, sizeof( ENCRYPTION_ALGORITHM ));
    (*pp_algorithm)->create_key = encryption_algorithm_create_key_2;
    (*pp_algorithm)->encrypt = encryption_algorithm_encrypt_2;
    (*pp_algorithm)->decrypt = encryption_algorithm_decrypt_2;
}
