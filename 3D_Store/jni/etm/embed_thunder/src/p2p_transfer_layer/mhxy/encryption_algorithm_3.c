...
BOOL encryption_algorithm_create_key_3(ENCRYPTION_ALGORITHM *p_algorithm, const _u8 *data, _u32 len, void *key, _u32 *p_key_len )
{
    // 创建首部
    if (len == 0)
    {
        ...
        // 加密方法
        _u32 head = ((8192*3 + rand() % 8192) << 16) + rand() % 65536; // [0x60000000, 0x7FFFFFFF]
...
BOOL encryption_algorithm_encrypt_3(ENCRYPTION_ALGORITHM *p_algorithm, _u8 *in_buffer, _u32 len )
{
    ...
    for (i=0; i<len; i++)
    {
        ...
        in_buffer[i] = p_algorithm->_key[p_algorithm->_key_pos] - in_buffer[i];
...
BOOL encryption_algorithm_3_new(struct tagENCRYPTION_ALGORITHM ** pp_algorithm)
{
    *pp_algorithm = (ENCRYPTION_ALGORITHM *)malloc(sizeof( ENCRYPTION_ALGORITHM ));
    ...
    (*pp_algorithm)->create_key = encryption_algorithm_create_key_3;
    (*pp_algorithm)->encrypt = encryption_algorithm_encrypt_3;
    (*pp_algorithm)->decrypt = encryption_algorithm_decrypt_3;
}
