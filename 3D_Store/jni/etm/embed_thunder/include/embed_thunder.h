...
typedef struct t_et_hsc_info
{
	uint64				_product_id;
	uint64				_sub_id;
	int32				_channel_type;// 通道类型: 0=商城免费3次 1=商城绿色通道 2=vip高速通道 -1=不可用
	ET_HSC_STAT 		_stat;
	u32 				_res_num;
	uint64 				_dl_bytes;
	u32 				_speed;
...
/* 尝试创建URL新任务,如果下载库正忙(比如正在删除一个大任务),则返回EBUSY,否则,执行et_create_new_task_by_url */
...
/*
  ...
  *       3.另外需要特别注意的是，参数file_name要与该任务在用et_create_new_task_by_url创建时传入的file_name_for_user的文件名部分一致，但由于文件的扩展名可能被下载库自动修改，
  		因此为了确保下载库在启动该续传任务时能准确地找到与之对应的.rf和.rf.cfg文件,一个比较保险的方法就是在用et_create_new_task_by_url创建任务的时候，当任务信息中的
  		ET_TASK结构体中的_file_create_status为ET_FILE_CREATED_SUCCESS的时候，调用et_get_task_file_name函数得到任务的最终文件名，再去除后缀.rf，则得到启动续传任务时的正确文件名.
...
/*
  ...
  *       2.另外需要特别注意的是，参数file_name要与该任务在用et_create_new_task_by_tcid创建时传入的file_name的文件名部分一致，但由于文件的扩展名可能被下载库自动修改，
  		因此为了确保下载库在启动该续传任务时能准确地找到与之对应的.rf和.rf.cfg文件,一个比较保险的方法就是在用et_create_new_task_by_tcid创建任务的时候，当任务信息中的
  		ET_TASK结构体中的_file_create_status为ET_FILE_CREATED_SUCCESS的时候，调用et_get_task_file_name函数得到任务的最终文件名，再去除后缀.rf，则得到启动续传任务时的正确文件名.
  */
...
/* 设置任务的写磁盘方式(默认为0)
	0 - 预分配方式(即一开始就根据文件大小创建一个空文件)
	1 - 自增长方式，乱序边下边写
 	2 - 自增长方式，顺序边下边写
 	3 - 自增长方式，乱序边下边写,最后再调成顺序 */
...
/*
  ...
  * 返回值: 0    成功
			   ...
			   4110  任务还没有运行
 			   4112  非法参数，file_name_buffer不能为空
                        ...
                        4215 buffer_size过小
  *
  * 注意：1.如果在ET_TASK_RUNNING状态下调用该接口，返回的为临时文件名(*.rf)，在任务成功后调用则返回最终文件名。
  ...
  *			调用该接口获得任务的正确文件名，并保存起来以被续传用。注意界面程序要去除文件名后缀.rf得到正确的文件名。 
...
/*
  * 删除指定的文件所匹配的临时文件，如 file_name.rf和file_name.rf.cfg文件。 
  * 返回值: 0    成功
  *
  * 注意：这个函数的调用与任务无关。
  */
...
typedef struct t_et_http_param
{
	...
	uint64  _range_from;			/* RANGE 起始位置*/
	uint64  _range_to;			/* RANGE 结束位置*/
	
	uint64  _content_len;			/* Content-Length:*/
	
	_BOOL  _send_gzip;			/* 是否发送压缩数据*/
	_BOOL  _accept_gzip;			/* 是否接受压缩文件 */
	
	u8 * _send_data;				/* 指向需要上传的数据*/
	u32  _send_data_len;			/* 需要上传的数据大小*/

	void * _recv_buffer;			/* 用于接收服务器返回的响应数据的缓存*/
	u32  _recv_buffer_size;		/* _recv_buffer 缓存大小*/
	...
	int32 _priority;				/* -1:低优先级,不会暂停下载任务; 0:普通优先级,会暂停下载任务;1:高优先级,会暂停所有其他的网络连接 【暂未实现】 */
} ET_HTTP_PARAM;
/* HTTP会话接口的回调函数参数*/
...
typedef struct t_et_http_call_back
{
	...
	u8 * _sent_data;			/* _type==EHCT_NOTIFY_SENT_DATA时有效, 数据分步上传时,指向已经上传的数据*/
	u32   _sent_data_len;		/* 已经上传的数据长度 */
	
	void ** _recv_buffer;			/* _type==EHCT_GET_RECV_BUFFER时有效, 数据分步下载时,指向用于接收数据的缓存*/
	u32  * _recv_buffer_len;		/* 缓存大小 */
	
	u8 * _recved_data;			/* _type==EHCT_PUT_RECVED_DATA时有效, 数据分步下载时,指向已经收到的数据*/
...
typedef struct t_et_search_server
{
         ...
         ET_FIND_SERVER_CALL_BACK_FUNCTION _find_server_callback_fun;             /* 每找到一个服务器，et就回调一下该函数，将该服务器的信息传给UI */
         ET_SEARCH_FINISHED_CALL_BACK_FUNCTION _search_finished_callback_fun;     /* 搜索完毕 */
} ET_SEARCH_SERVER;
