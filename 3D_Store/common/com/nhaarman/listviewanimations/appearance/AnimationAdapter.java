...
public abstract class AnimationAdapter extends BaseAdapterDecorator {
    ...
    private void animateViewIfNecessary(final int position, final View view, final ViewGroup parent) {
        ...
        Log.d("animateViewIfNecessary", "mGridViewPossiblyMeasuring = "+mGridViewPossiblyMeasuring);
         ...
         * 解决GridView 只有一项的时候每次Notify都会重新动画的问题
         * */
        if(mViewAnimator.getLastAnimatedPosition() == -1){
        	BaseAdapter adapter = getDecoratedBaseAdapter();
        	if(adapter instanceof AnimDeleteAdapter){
        		if(((AnimDeleteAdapter)adapter).hasNotifiedOnce()){
        			return;
        		}
        	}
        }
