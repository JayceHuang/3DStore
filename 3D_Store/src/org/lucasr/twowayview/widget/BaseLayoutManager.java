...
public abstract class BaseLayoutManager extends TwoWayLayoutManager {
	...
	protected void pushChildFrame(ItemEntry entry, Rect childFrame, int lane, int laneSpan, Direction direction) {
	...
	private void popChildFrame(ItemEntry entry, Rect childFrame, int lane, int laneSpan, Direction direction) {
		for (int i = lane; i < lane + laneSpan; i++) {
			final int spanMargin;
			if (entry != null && direction != Direction.END) {
				spanMargin = entry.getSpanMargin(i - lane);
	...
	protected void detachChild(View child, Direction direction) {
		...
		popChildFrame(getItemEntryForPosition(position), mChildFrame, mTempLaneInfo.startLane, getLaneSpanForChild(child), direction);
	}
