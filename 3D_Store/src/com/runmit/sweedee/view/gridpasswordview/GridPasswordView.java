package com.runmit.sweedee.view.gridpasswordview;
...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        textColor = ta.getColorStateList(R.styleable.gridPasswordView_itemTextColor);
        if (textColor == null){
        	textColor = ColorStateList.valueOf(getResources().getColor(android.R.color.primary_text_light));
        }
        int textSize = ta.getDimensionPixelSize(R.styleable.gridPasswordView_itemTextSize, -1);
        if (textSize != -1) {
        ...
        lineColor = ta.getColor(R.styleable.gridPasswordView_itemLineColor, DEFAULT_LINECOLOR);
        gridColor = ta.getColor(R.styleable.gridPasswordView_gridColor, DEFAULT_GRIDCOLOR);
        lineDrawable = ta.getDrawable(R.styleable.gridPasswordView_itemLineColor);
    ...
    private boolean forceInputViewGetFocus() {
        inputView.setFocusable(true);
        inputView.setFocusableInTouchMode(true);
        boolean isSuccess = inputView.requestFocus();
        Log.d(VIEW_LOG_TAG, "isSuccess="+isSuccess);
        InputMethodManager imm = (InputMethodManager) getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
        isSuccess = imm.showSoftInput(inputView, InputMethodManager.SHOW_IMPLICIT);
        Log.d(VIEW_LOG_TAG, "isSuccess="+isSuccess);
        return isSuccess;
    }
